...
export default function AdminDashboard() {
  ...
  const fetchData = () => {
    axios.get("/admin/products").then(res => setProducts(res.data)).catch(() => {});
    axios.get("/admin/orders").then(res => setOrders(res.data)).catch(() => {});
      ...
      <div className="admin-grid">
        ...
        <div className="admin-card">
          ...
          <ul className="admin-list">
            ...
            {products.length === 0 && <li style={{color:"var(--c-text-light)",fontStyle:"italic"}}>No products yet</li>}
          </ul>
        </div>
        <div className="admin-card">
          <h3>Orders ({orders.length})</h3>
          <ul className="admin-list">
            {orders.map(o => <li key={o._id}><span>{o.customerName}</span><span>₹{o.totalAmount}</span></li>)}
