...
export default function Checkout() {
  ...
  if (submitted) {
    const waText = encodeURIComponent(
      `Hi! I placed an order on V Creations.%0AOrder: ${lastOrder?._id?.slice(-8)?.toUpperCase() || ""}%0ATotal: ₹${lastOrder?.totalAmount || total}%0APlease confirm payment.`
    );
    ...
    return (
      <div style={{textAlign:"center",padding:"40px 0",maxWidth:480,margin:"0 auto"}}>
        ...
        <p style={{color:"#666",marginBottom:24}}>
          Order ID: <strong>#{lastOrder?._id?.slice(-8)?.toUpperCase() || "—"}</strong>
        </p>
        <div style={{background:"#f5faff",border:"2px solid var(--c-primary)",borderRadius:12,padding:24,marginBottom:24,textAlign:"left"}}>
          <h3 style={{color:"var(--c-primary)",marginBottom:12,fontSize:18}}>💳 Pay Now</h3>
          <p style={{marginBottom:8}}>Pay via UPI to:</p>
          <div style={{fontSize:22,fontWeight:800,color:"#111",background:"white",padding:"10px 16px",borderRadius:8,border:"1px solid #ddd",marginBottom:16,textAlign:"center",letterSpacing:1}}>
            {config.upiId}
          </div>
          <p style={{fontSize:14,color:"#555",marginBottom:16}}>
            After payment, send the screenshot on WhatsApp for confirmation.
          </p>
          <a href={waUrl} target="_blank" rel="noopener noreferrer"
            className="btn"
            style={{display:"inline-flex",alignItems:"center",gap:8,background:"#25D366",color:"white",fontWeight:700,fontSize:15,padding:"12px 24px",borderRadius:8,textDecoration:"none"}}>
